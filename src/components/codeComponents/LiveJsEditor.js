    ...
    <>
      {!hideCode && (
        <>
          <SnippetInfo
            language={language}
            editable={!editingDisabled && shouldShowEditor}
            scripts={scripts}
            live={true}
          />
      ...
      <JsComponent
        ...
        scripts={scripts}
        hideControls={editingDisabled || hideCode}
        autorun={autorun}
        hideCode={hideCode}
        linkId={linkId}
      />
...
function JsComponent({
  ...
  function evaluateCode() {
    ...
    try {
      ...
      if (scriptsNotLoaded.length > 0) {
        return console.blog(
          `Script Execution Stopped: Failed to load ${scriptsNotLoaded.join(
            ', '
          )}`
        )
      }
  ...
  useEffect(() => {
    ...
    if (
      scripts.filter(
        name => !globalScripts[name] || globalScripts[name].loading
      ).length > 0
    )
      return
    evaluateCode(code)
  ...
  return (
    <>
      ...
      {logs.filter(logObj => logObj.uniqueIdentifier === id.current).length >
        0 && (
        <pre
          style={{
            whiteSpace: 'pre-wrap',
        ...
        >
          {logs
            .filter(logObj => logObj.uniqueIdentifier === id.current)
            .map(({ logs }) =>
              logs.map(
                logArr =>
                  logArr
                    .map(logItem => JSON.stringify(logItem, null, 2))
                    .join('\n') + '\n'
              )
            )}
        </pre>
