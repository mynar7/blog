...
function ResetButton({ initialCode, update }) {
  ...
  return (
    <button style={{ marginTop: rhythm(0.5) }} onClick={performUpdate}>
      Reset Code
    </button>
  )
}
...
function LiveReactEditor({
  ...
  useEffect(
    () => {
      ...
      failedScripts.current = scripts.filter(
        (name) => globalScripts[name] && globalScripts[name].failed
      )
      if (ready) {
